...
Or point to your config explicitly. Confirm in the logs that the monitor connects to Anvil and processes blocks.

***Tip:*** While the monitor is running, the Anvil terminal will show repeated RPC calls (`eth_blockNumber`, `eth_getBlockByNumber`, `eth_getLogs`, etc.). This is normal; the monitor polls every second.
...
* Use [Testing your configuration](/monitor/1.0.x#testing-your-configuration) (e.g. `--monitor-path` and `--network` / `--block`) to test against specific blocks.
* See the [Quickstart](/monitor/1.0.x/quickstart) and main [Configuration](/monitor/1.0.x#configuration-guidelines) docs for more options.
