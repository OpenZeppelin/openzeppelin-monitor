...
To iterate on monitor behavior without using public testnets, you can run a local EVM node with [Foundry](https://book.getfoundry.sh/) and point the monitor at it. See [Local EVM Testing](/monitor/1.0.x/local-evm-testing) for setup, deployment, and triggering conditions.
