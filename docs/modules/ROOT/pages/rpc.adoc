...
The RPC client uses `reqwest_retry` middleware with exponential backoff to handle transient failures in network requests. This is implemented separately from the endpoint rotation mechanism.

===== Configuration

The retry mechanism is configured in the transport client with:

[source,rust]
----
// Default retry policy with 2 retries (3 total attempts)
...
* Maximum of 2 retries per request
* Same URL is used for retries

2. *Endpoint Rotation*
* Handles persistent failures (e.g., 429 rate limits)
* Switches to different URLs in the fallback pool
* Triggered by specific HTTP status codes
* Maintains a pool of fallback URLs
