...
* Trigger conditions (scripts to execute that evaluate every monitor match to determine if it should launch a trigger or not)
...
* Input: The filter script receives a JSON object containing two fields: `monitor_match` and `args`. The `monitor_match` field contains the monitor match data and the `args` field contains the arguments of the script.
...
|execution_order
|Number
|The order of the execution of the script in case there are multiple trigger conditions
