...
For users who want to get started quickly without manually configuring files, we provide an automated setup script that handles the entire installation and configuration process.

=== Automated Setup

The `setup_and_run.sh` script provides a one-command solution to:

* **Build the monitor application** from source
* **Copy example configurations** from the `examples/` directory to `config/`
...
* **Validate all configurations** to ensure they're properly set up
...
The script will guide you through the setup process with colored output and provide clear instructions for next steps.

=== After Running the Script

Once the setup script completes successfully, you'll have:

* A fully built and configured OpenZeppelin Monitor
* Example configurations ready for customization
* Clear guidance on setting up RPC URLs and notification channels

You can then:

. **Edit your `.env` file** with your specific RPC URLs and notification settings
. **Customize the copied configurations** in the `config/` directories
. **Run the monitor** with `./openzeppelin-monitor`

[TIP]
====
The setup script creates working configurations with placeholder values. Remember to update your `.env` file with actual RPC endpoints and notification credentials before starting real monitoring.
====
...
[NOTE]
====
Video demonstrations are coming soon! We're preparing comprehensive tutorials covering:

* Setting up your first monitor
* Configuring multi-chain monitoring
...
Watch for large USDC transfers across multiple networks with real-time Slack notifications.

**DeFi Protocol Monitoring**
Track liquidity changes, large swaps, and governance events on popular DeFi protocols.

**Smart Contract Security**
Monitor for suspicious activities, large withdrawals, and unusual contract interactions.

**Cross-Chain Activity**
Track bridge transactions and cross-chain token movements in real-time.
...
Have feature requests or want to contribute? Join our community discussions on link:https://github.com/OpenZeppelin/openzeppelin-monitor/discussions[GitHub Discussions] or link:https://t.me/openzeppelin_tg/4[Telegram].
