...
. If you are directly using `openzeppelin-monitor` binary/cli. Please see list of options using:
+
[source,bash]
----
...
. To turn on log file mode you will need to pass `--log-file`.

. To turn on metrics server, you will need to pass `--metrics`.

=== Docker Installation:

. Based on your `.env` file, docker compose may or may not start the metrics server (within monitor app container), prometheus and grafana.

. If you want to start the metrics server, prometheus and grafana, make sure to set `METRICS_ENABLED=true` in your `.env` file.

. If you want to start the services using make file target, you can use the following command to start the services:
+
[source,bash]
----
# To Build image and run containers
cargo make docker-compose-up
----

[NOTE]
====
By default docker compose command uses Dockerfile.development to build the image. If you want to use Dockerfile.production, you can set: `DOCKERFILE=Dockerfile.production` before running `cargo make docker-compose-up`.
====

. We have a make file target to start the services with docker compose with metrics profile based on your `.env` file.

. For metrics server you will need to make sure `METRICS_ENABLED=true` is set in your `.env` file. If you want to start the services directly using docker compose, you can use the following command:
+
[source,bash]
----
...
. To check the logs of the services/containers, run the following command ( if running in default/stdout mode ):
+
[source,bash]
----
...
[NOTE]
====
By default we won't persist monitor data between container restarts, to do so:
[source,bash]
----
