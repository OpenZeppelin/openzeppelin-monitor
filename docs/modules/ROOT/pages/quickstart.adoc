= Quick Start Guide
:description: Get started monitoring USDC transfers on Ethereum mainnet

This guide will walk you through setting up OpenZeppelin Monitor to track large USDC transfers on Ethereum mainnet.
...
This link:https://github.com/OpenZeppelin/openzeppelin-monitor/blob/main/config/monitors/evm_transfer_usdc.json.example[configuration^] monitors USDC transfers over 10,000 USDC (10 million base units, accounting for 6 decimals). You can customize the notification channels by modifying the `triggers` array at the bottom of the configuration, or keep the default Slack and email notifications that we'll set up in the next section.
