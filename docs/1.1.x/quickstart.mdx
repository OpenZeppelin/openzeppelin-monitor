...
For detailed configuration options, see [Basic Configuration](/monitor/1.1.x#basic_configuration).
...
* Now, you can also filter by parameters event’s name (Stellar Protocol 23 has introduced this new feature). See this [section](/monitor/1.1.x#stellar_specifics) for more details
...
To iterate on monitor behavior without using public testnets, you can run a local EVM node with [Foundry](https://book.getfoundry.sh/) and point the monitor at it. See [Local EVM Testing](/monitor/1.1.x/local-evm-testing) for setup, deployment, and triggering conditions.

### Testing and Validation

* [Test your configuration](/monitor/1.1.x#testing_your_configuration) against specific block numbers
* Verify your RPC endpoints are working correctly
* Test notification channels with small transactions

### Security and Best Practices

* [Configure secure secret management](/monitor/1.1.x#secret_management) for sensitive data
...
* Review the [User Documentation](/monitor/1.1.x) for detailed configuration options
