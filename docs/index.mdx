...
| `**transaction.hash**` | Hash of the transaction (or `transaction.signature` for Solana) |
| `**network.slug**` | Network identifier (e.g., `ethereum_mainnet`, `stellar_mainnet`) |
...
| `**transaction.to**` | Recipient address (omitted for contract creation) |
| `**transaction.value**` | Transaction value in wei |
| `**transaction.blockNumber**` | Block number (omitted for pending transactions) |
| `**transaction.nonce**` | Transaction nonce |
| `**transaction.gasLimit**` | Gas limit for the transaction |
| `**transaction.gasPrice**` | Gas price (omitted for EIP-1559 transactions) |
| `**transaction.input**` | Transaction input/calldata |
...
| **Variable** | **Description** |
| --- | --- |
| `**transaction.ledgerSequence**` | Ledger sequence number containing the transaction |
...
| **Variable** | **Description** |
| --- | --- |
| `**transaction.signature**` | Transaction signature (equivalent to hash) |
| `**transaction.slot**` | Slot number containing the transaction |
| `**functions.[index].signature**` | Instruction signature |
| `**functions.[index].args.[param]**` | Instruction parameters by name |
| `**events.[index].signature**` | Log/event signature |
| `**events.[index].args.[param]**` | Log/event parameters by name |
...
##### Full Monitor Match Access

For advanced use cases requiring complete transaction data, the full `MonitorMatch` object is available via the `monitor_match` prefix. This provides access to all fields that script triggers receive.

| **Variable Pattern** | **Description** |
| --- | --- |
| `**monitor_match.EVM.***` | Full EVM monitor match data |
| `**monitor_match.Stellar.***` | Full Stellar monitor match data |
| `**monitor_match.Solana.***` | Full Solana monitor match data |
| `**monitor_match.Midnight.***` | Full Midnight monitor match data |

Example paths for EVM:
- `monitor_match.EVM.transaction.hash` - Transaction hash
- `monitor_match.EVM.logs.0.address` - First log's contract address
- `monitor_match.EVM.network_slug` - Network identifier
- `monitor_match.EVM.monitor.name` - Monitor name from config

###### MonitorMatch Object Structure

The `MonitorMatch` object varies by blockchain type. Below are the structures for each supported blockchain:

**EVM MonitorMatch** (`monitor_match.EVM.*`)
```json
{
  "monitor": {
    "name": "string",
...
> **Note:** The `receipt` field may be `null` if the monitor doesn't require receipt data. Receipt is only fetched when the monitor has transaction conditions using `gas_used` in an expression or when status validation is needed without logs.

**Stellar MonitorMatch** (`monitor_match.Stellar.*`)
```json
{
  "monitor": {...},
  "transaction": {
  ...
  "network_slug": "stellar_mainnet",
  "matched_on": {...},
  "matched_on_args": {...}
}
```

**Solana MonitorMatch** (`monitor_match.Solana.*`)
```json
{
  "monitor": {...},
  "transaction": {
  ...
  "network_slug": "solana_mainnet",
  "matched_on": {...},
  "matched_on_args": {...}
}
```

**Midnight MonitorMatch** (`monitor_match.Midnight.*`)
```json
{
  "monitor": {...},
  "transaction": {
