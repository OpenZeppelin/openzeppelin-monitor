...
| `**fee_payer**` | `pubkey` | Address that paid the transaction fee (first account in account_keys). Only present if defined. |
| `**accounts**` | `string` | All accounts involved in the transaction, including addresses loaded from lookup tables (ALTs). Format: `\|account1\|account2\|...\|` with pipe delimiters. Use `accounts contains "\|<address>\|"` for exact matching. |
